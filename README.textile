...
HBO is designed to make backing up your Heroku applications easy. It is designed to be Heroku deployable and uses Amazon S3 as the permanent storage for your backups (heroku bundles). Backups are scheduled using the free daily cron addon. HBO also includes a simple web interface for viewing the contents of the S3 bucket where you keep your backups, and for triggering ad-hoc manual backups. 
...
* Run @heroku create@ on in the application root directory
...
* Animate a bundle directly from the web UI (when they fix the animate feature)
* Pagination of bundles, as there are bound to be many.